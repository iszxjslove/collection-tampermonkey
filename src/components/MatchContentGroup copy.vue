...
const actions = {
  ...
  save() {
    actionPost("save", {
      contents: JSON.parse(JSON.stringify(elements.contentsComputed)),
    });
  },
  delete(index) {
...
const keydown = {
  ...
  action(action, key) {
    let params = [];
    if (typeof keydown.parse[action] === "function") {
      params = keydown.parse[action].call(this, key);
    }
    ...
    for (const key in keydown.combination) {
      ...
      for (const action in keydown.combination[key]) {
        if (
          ...
          keydown.action.call(event, action, event.key.toLowerCase());
        }
      }
    }
  },
